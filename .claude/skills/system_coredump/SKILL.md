...
`ansible/roles/system_coredump/README.md` documents this role's full
...
git log -1 --format='%cI %h %s' -- ansible/roles/system_coredump/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/system_coredump/policy-<short-desc>` or
  `soe/system_coredump/workload-<short-desc>`), same branch + PR workflow as
