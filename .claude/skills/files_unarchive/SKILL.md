...
`ansible/roles/files_unarchive/README.md` documents this role's full
...
git log -1 --format='%cI %h %s' -- ansible/roles/files_unarchive/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/files_unarchive/policy-<short-desc>` or
  `soe/files_unarchive/workload-<short-desc>`), same branch + PR workflow as
...
invoke `files_unarchive` in this repo. Before running it, either:

1. **Add it to an existing playbook** — most often
   `ansible/configure_rhel.yml`'s `roles:` list (uncomment-style, with a
   `vars:` entry for `files_unarchive` set to what's actually needed), if what's wanted is
...
2. **Write a small ad hoc playbook** for the one-off task, same shape as
   ...
   `vars:` block plus a `roles: [files_unarchive]` list), if it's a one-time or
...
the same way as any other role — `--tags files_unarchive` against whichever playbook now
contains it, `--check --diff` first:

```
ansible-playbook ansible/<playbook>.yml --tags files_unarchive --check --diff   # audit
ansible-playbook ansible/<playbook>.yml --tags files_unarchive                  # remediate
```
