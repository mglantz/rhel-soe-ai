...
`ansible/roles/multipath_setup/README.md` documents this role's full
...
git log -1 --format='%cI %h %s' -- ansible/roles/multipath_setup/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/multipath_setup/policy-<short-desc>` or
  `soe/multipath_setup/workload-<short-desc>`), same branch + PR workflow as
