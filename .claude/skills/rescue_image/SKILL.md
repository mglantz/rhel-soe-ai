...
`ansible/roles/rescue_image/README.md` documents this role's full
...
git log -1 --format='%cI %h %s' -- ansible/roles/rescue_image/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/rescue_image/policy-<short-desc>` or
  `soe/rescue_image/workload-<short-desc>`), same branch + PR workflow as
...
> list variable (`when: "'<name>' in configure_rhel_domains"`), and `rescue_image`
...
> `-e`, e.g.
> `-e '{"configure_rhel_domains": [...the default 20..., "rescue_image"]}'`
...
> `-e` replaces the variable's value, it doesn't merge into it). `--tags rescue_image`
