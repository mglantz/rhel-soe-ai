...
`ansible/roles/domain_ad/README.md` documents this role's full
...
git log -1 --format='%cI %h %s' -- ansible/roles/domain_ad/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/domain_ad/policy-<short-desc>` or
  `soe/domain_ad/workload-<short-desc>`), same branch + PR workflow as
