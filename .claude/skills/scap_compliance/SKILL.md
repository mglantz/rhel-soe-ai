...
git log -1 --format='%cI %h %s' -- ansible/roles/scap_compliance/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/scap_compliance/policy-<short-desc>` or
  `soe/scap_compliance/workload-<short-desc>`), same branch + PR workflow as
