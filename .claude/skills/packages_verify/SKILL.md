...
`ansible/roles/packages_verify/README.md` documents this role's full
...
git log -1 --format='%cI %h %s' -- ansible/roles/packages_verify/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/packages_verify/policy-<short-desc>` or
  `soe/packages_verify/workload-<short-desc>`), same branch + PR workflow as
