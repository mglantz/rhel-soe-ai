...
`ansible/roles/files_fetch/README.md` documents this role's full
...
git log -1 --format='%cI %h %s' -- ansible/roles/files_fetch/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/files_fetch/policy-<short-desc>` or
  `soe/files_fetch/workload-<short-desc>`), same branch + PR workflow as
