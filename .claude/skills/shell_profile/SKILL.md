...
git log -1 --format='%cI %h %s' -- ansible/roles/shell_profile/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/shell_profile/policy-<short-desc>` or
  `soe/shell_profile/workload-<short-desc>`), same branch + PR workflow as
