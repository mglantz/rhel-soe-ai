...
`ansible/roles/accounts_local/README.md` documents this role's full
...
git log -1 --format='%cI %h %s' -- ansible/roles/accounts_local/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/accounts_local/policy-<short-desc>` or
  `soe/accounts_local/workload-<short-desc>`), same branch + PR workflow as
