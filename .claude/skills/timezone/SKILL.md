...
`ansible/roles/timezone/README.md` documents this role's full
...
git log -1 --format='%cI %h %s' -- ansible/roles/timezone/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/timezone/policy-<short-desc>` or
  `soe/timezone/workload-<short-desc>`), same branch + PR workflow as
