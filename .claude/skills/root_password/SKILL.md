...
git log -1 --format='%cI %h %s' -- ansible/roles/root_password/
```
...
- **Relevant** — propose a concrete change on a branch
  (`soe/root_password/policy-<short-desc>` or
  `soe/root_password/workload-<short-desc>`), same branch + PR workflow as
